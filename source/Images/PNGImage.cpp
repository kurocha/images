...
namespace Images
{
	...
	struct PNGProgressiveReader {
		...
		void read(Data * data, std::size_t block_size) {
			std::size_t offset = 0;
			...
			while (!stop) {
				...
				process_data(data->begin() + offset, size);
				offset += size;
			}
		}
		...
		PNGProgressiveReader() {
			png_reader = png_create_read_struct(PNG_LIBPNG_VER_STRING, nullptr, png_error, nullptr);
			assert(png_reader != nullptr && "png_create_read_struct returned nullptr!");

			png_info = png_create_info_struct(png_reader);
			assert(png_info != nullptr && "png_create_info_struct returned nullptr!");
		}
		
		~PNGProgressiveReader() {
			png_destroy_read_struct(&png_reader, &png_info, (png_infopp)nullptr);
		}
	};
	...
	struct PNGHeaderReader : public PNGProgressiveReader {
		...
		PNGHeaderReader() {
			png_set_progressive_read_fn(png_reader, (void *)this, info_callback, nullptr, nullptr);
		}
	};
	...
	{
		assert(png_check_sig((png_byte*)data->begin(), 8));
		
		PNGHeaderReader reader;
		reader.read(data, 1024);
	...
	Owned<Data> PNGImage::save(PixelBufferLayout2D layout, const Byte * data)
	{
		...
		auto result_data = shared<DynamicBuffer>();
		
		// Data pointers, one per row:
		auto rows = layout.generate_row_pointers(data);
		
		try {
			png_writer = png_create_write_struct(PNG_LIBPNG_VER_STRING, nullptr, png_error, nullptr);
			assert(png_writer != nullptr && "png_create_write_struct returned nullptr!");
			...
			png_set_write_fn(png_writer, static_cast<void *>(result_data.get()), png_write_to_buffer, nullptr);
			...
			png_write_end(png_writer, nullptr);
